...
import { UserFollowData } from './FollowingList';
...
export default function FollowerList() {
  ...
  useEffect(() => {
    axios
      .post(`${BACKEND_URL}/api/follow/follower`, {
        userId: userId,
      })
      .then(async (response) => {
        const data = await response.data;
        setFollowData(data.follower);
      });
  }, [userId]);
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div className="w-full flex justify-between">
          ...
          <div className="flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
            <div>
              <span className="font-medium">{followData.follow.length}</span> follower
            </div>
            <div>
              <Button
                variant="secondary"
